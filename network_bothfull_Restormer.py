...
class Network(BaseNetwork):
    def __init__(self, unet, beta_schedule, module_name='Restormer_net', **kwargs):
        ...
        if module_name == 'sr3':
            ...
            from .convnext_unet.unet import ConvNeXtV2
        elif module_name == 'Restormer_net':
            from .restormer.restormer_arch import Restormer
            #print(True)
        else :
            from .restormer.restormer_arch import Restormer
        #self.denoise_fn = Restormer()
        self.denoise_fn = Restormer(**unet)
    ...
    def forward(self, y_0, y_cond=None, mask=None, noise=None,ssim =ssim):
        ...
        if mask is not None:
            noise_hat = self.denoise_fn(torch.cat([y_cond, y_noisy*(mask + 0.25)+(0.75 - mask)*y_0], dim=1), sample_gammas)#
            ...
            loss = loss_G_Ang + 4*loss# + 5*loss_gradient_SR
        ...
        print('loss_G_Ang',loss_G_Ang)
        print('4*loss',4*loss)
        #print('5*loss_gradient_SR',5*loss_gradient_SR)
        #combined_loss = loss_G_Ang + loss_ssim
