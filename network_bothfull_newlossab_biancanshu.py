...
class Network(BaseNetwork):
    def __init__(self, scunet, beta_schedule, module_name='scunet', **kwargs):
        super(Network, self).__init__(**kwargs)
        if module_name == 'sr3':
            from .sr3_modules.unet import UNet
        elif module_name == 'guided_diffusion':
            from .guided_diffusion_modules.unet import UNet
        elif module_name == 'scunet':
            from .scunet.scunet import SCUNet

        #self.denoise_fn = UNet(**unet)
        self.denoise_fn = SCUNet(**scunet)
    ...
    def update_mask(self, mask, learning_param , initial_values , tolerance=0.01):
        ...
        # 计算新值
            gengxin = 1- (3/2*i)
            new_value = gengxin + i * learning_param
