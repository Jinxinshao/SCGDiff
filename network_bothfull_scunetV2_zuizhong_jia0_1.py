...
class Network(BaseNetwork):#SCUnet_V2
    ...
    def update_mask(self, mask, learning_param , initial_values , tolerance=0.01):
        ...
        # 计算新值
            new_value = 1 - 1.5 * learning_param +i* learning_param
    ...
    def forward(self, y_0, y_cond=None, mask=None, noise=None,ssim =ssim):
        ...
        if mask is not None:
            ...
            #print(mean_all)\
            # 在这个思路上还可以做两个消融实验
            noise_hat = self.denoise_fn(torch.cat([y_cond, y_noisy*(mask + 1 -mean_all + 0.1)+(mean_all - mask - 0.1)*y_0], dim=1), sample_gammas)#
        ...
        # 使用新的权重组合loss
        print('1.0*loss_G_Ang', 1.0*loss_G_Ang)
        print('9990.0*loss_ssim',990.0*loss_ssim)
        print('9.0*loss_gradient_SR',9.0*loss_gradient_SR)
        combined_loss = 9990.0*loss_ssim + 1.0*loss_G_Ang +  9.0*loss_gradient_SR
        #combined_loss = loss 
