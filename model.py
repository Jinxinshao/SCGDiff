...
from core.logger import LogTracker,LogTracker_new
import copy
#from thop import profile
import thop
#from ptflops import get_model_complexity_info
#from pytorch_flops import count_flops
...
class Palette(BaseModel):
    def __init__(self, networks, losses, sample_num, task, optimizers, ema_scheduler=None, **kwargs):
        ...
        self.some_key = ['learning_param']
        self.some_value = [0.1]  # 这些初始值将用于后续更新
        ...
        self.train_metrics_new = LogTracker_new(self.some_key, self.some_value, phase='train')
    ...
    def train_step(self):
        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            self.optG.step()
            with torch.no_grad():
                #self.optG['train']['learning_param'] = self.netG.module.learning_param
                self.netG.module.learning_param.clamp_(self.min_value, self.max_value)
                #对学习参数进行限幅
                print(self.netG.module.learning_param)
            self.iter += self.batch_size
            self.writer.set_iter(self.epoch, self.iter, phase='train')
            self.train_metrics.update(self.loss_fn.__name__, loss.item())
            self.train_metrics_new.update('learning_param', self.netG.module.learning_param)
            if self.iter % self.opt['train']['log_iter'] == 0:
                for key, value in self.train_metrics.result().items():
                    self.logger.info('{:5s}: {}\t'.format(str(key), value))
                    self.writer.add_scalar(key, value)
                for key, value in self.train_metrics_new.result().items():
                    self.logger.info('{:5s}: {}\t'.format(str(key), value))
                    self.writer.add_scalar(key, value)                    
        ...
        for scheduler in self.schedulers:
            scheduler.step()
        combined_results = {**self.train_metrics.result(), **self.train_metrics_new.result()}

        return combined_results 

        #return self.train_metrics.result()+self.train_metrics_new.result()
            ...
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                if self.opt['distributed']:
                    if self.task in ['inpainting','uncropping']:
                        ...
                        # 这里改动了，原版是下面改动的内容
                        #self.output, self.visuals = self.netG.module.restoration(self.cond_image, sample_num=self.sample_num)
                        #flops, params = profile(self.netG.module.restoration, inputs=self.cond_image)
                    ...
                    if self.task in ['inpainting','uncropping']:
                        ...
                        # 这里改动了，原版是下面改动的内容
                        #self.output, self.visuals = self.netG.restoration(self.cond_image, sample_num=self.sample_num)
                        #flops, params = profile(self.netG.restoration, inputs=self.cond_image)
                        #print('FLOPs: %.4f G' % (flops / 1e9))
                        #print('model parameters: ', params)

                #flops, params = thop.profile(self.netG, inputs=(self.cond_image,))
    ...
    def save_best(self):
        """ load pretrained model and training state. """
        if self.opt['distributed']:
            netG_label = "Best_Network"
        else:
            netG_label = "Best_Network"
        self.save_best_network(network=self.netG, network_label=netG_label)
        if self.ema_scheduler is not None:
            self.save_best_network(network=self.netG_EMA, network_label=netG_label+'_ema')
